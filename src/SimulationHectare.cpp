	...
	if (!std::filesystem::exists(filepath) || !std::filesystem::is_regular_file(filepath)) {
		...
		if (file.is_open()) {
			...
			//	if (!read_and_validate_type(linestream, is_rel_site, "is_rel_site" + std::to_string(i+1), "char")) err++;
				...
				if (!read_and_validate_type(linestream, relnum, "release number" + std::to_string(i+1), "int")) err++;
		...
		*/
				...
				//	if (type == 3) {temp_rel_sites.push_back(i);}
					if (type == 4) 
					{
						release_sites.push_back(i);
						release_number.push_back(relnum);
				//		num_output_sites++;
					}
				//	if (type == 5) {num_output_sites++;}
				}
			}
			num_output_sites=*std::max_element(patch_index.begin(), patch_index.end());
		}
		file.close();
